...
public static class PlcAddressParser
{
    ...
    /// 注意：解析是**纯函数**（键 = 地址字符串本身，输入变了键就变，必然重新计算），
    /// 不存在"配置变更后旧结果被沿用"的可能——本方法唯一实际作用是**内存回收**，
    /// 在设备/报警/缺陷地址或连接档案配置变更后调用可以释放缓存占用（审查修复 2026-09-02，P1-7：
    /// 原注释错误地声称"必须调用否则旧地址结果被沿用"，与纯函数语义不符）。
    /// </summary>
