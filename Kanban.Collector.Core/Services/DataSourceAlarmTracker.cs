...
public sealed class DataSourceAlarmTracker
{
    ...
    /// </summary>
    public void Observe(Device device, DataSource source, DataSourceValue valueItem, int value, string shiftName, DateTime? sampledAt = null)
        => Observe(device, source, valueItem, new DataSourceRuntimeValue(DataSourceValueType.Int32, Int32Value: value), shiftName, sampledAt);

    public void Observe(
        Device device,
        DataSource source,
        DataSourceValue valueItem,
        DataSourceRuntimeValue value,
        string shiftName,
        DateTime? sampledAt = null)
    {
        valueItem.SetRuntimeValue(value, sampledAt);
