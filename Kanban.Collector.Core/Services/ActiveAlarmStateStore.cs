...
/// 边沿 Upsert 幂等，写入失败无需重试集合——下轮边沿/重建时自动覆盖。
/// </summary>
    ...
    : IActiveAlarmStateService
{
    ...
    public bool RemoveByDeviceId(string deviceId)
    {
        ...
        {
            using var ctx = db.CreateAlarmEventContext();
            var rows = ctx.ActiveAlarmStates.Where(e => e.DeviceId == deviceId).ToList();
            if (rows.Count == 0) return true;
            ctx.ActiveAlarmStates.RemoveRange(rows);
            ctx.SaveChanges();
            return true;
        }
        catch (Exception ex)
    ...
    public bool RemoveByAlarm(string deviceId, string alarmId)
    {
        ...
        {
            using var ctx = db.CreateAlarmEventContext();
            var rows = ctx.ActiveAlarmStates.Where(e => e.DeviceId == deviceId && e.AlarmId == alarmId).ToList();
            if (rows.Count == 0) return true;
            ctx.ActiveAlarmStates.RemoveRange(rows);
            ctx.SaveChanges();
            return true;
