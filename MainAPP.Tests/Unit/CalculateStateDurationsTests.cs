...
public class CalculateStateDurationsTests
{
    ...
    [Fact]
    public void NoTransitions_InitialState0_AccruesToOffline()
    {
        var from = new DateTime(2026, 7, 23, 8, 0, 0);
        var to = from.AddMinutes(30);
        var (run, alarm, paused, offline) = OeeCalculator.CalculateStateDurations(
    ...
    [Fact]
    public void OfflineState_AccruesToOfflineOnly()
    {
        var t0 = new DateTime(2026, 7, 23, 8, 0, 0);
        var transitions = new List<StatusTransitionRecord>
        {
            Tr(0, 0, t0.AddMinutes(10)) // 离线态切换
        };
        var (run, alarm, paused, offline) = OeeCalculator.CalculateStateDurations(
    ...
    public void FutureTo_IsTruncatedToNow()
    {
        ...
        // 被截断到 now：总时长 ≈ 1 小时（仅 OEE 三态，不含离线）
        var total = run + alarm + paused + offline;
