...
/// 含 Collector 采集进程需要的字段（轮询/批读/班次/PLC 连接）以及 Remote 展示端需要的界面语言；
/// 其他纯本地 UI 设置（主题/字号/日报等）不在此同步。
/// 可空字段 = 部分更新语义（只更新有值的字段；Collector 端合并后整体落盘 settings.json）。
/// </summary>
public sealed record CollectorSettingsDto
{
    /// <summary>界面语言（AppLanguage 枚举值），Remote 模式下由 Collector 作为展示端权威。</summary>
    ...
    /// <summary>数据源协议键；缺省为 plc。</summary>
    public string? PlcProtocolKey { get; init; }
...
public sealed record ConnectionProfileSettingsDto
{
    ...
    public int? PlcBrand { get; init; }
    public string? ProtocolKey { get; init; }
